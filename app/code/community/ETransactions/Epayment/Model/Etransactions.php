...
class ETransactions_Epayment_Model_Etransactions {
    ...
    protected function _callDirect($type, $amount, Mage_Sales_Model_Order $order, Mage_Sales_Model_Order_Payment_Transaction $transaction) {
        ...
        $callNumber = $transaction->getAdditionalInformation(ETransactions_Epayment_Model_Payment_Abstract::CALL_NUMBER);
        $transNumber = $transaction->getAdditionalInformation(ETransactions_Epayment_Model_Payment_Abstract::TRANSACTION_NUMBER);
    ...
    public function buildSystemParams(Mage_Sales_Model_Order $order, ETransactions_Epayment_Model_Payment_Abstract $payment) {
        $config = $this->getConfig();
        ...
        if (!isset($cards[$code])) {
            ...
            Mage::throwException(Mage::helper('etep')->__($message), $code);
        }
        $card = $cards[$code];
    ...
    public function checkUrls(array $urls) {
        ...
        // Here, there's a problem
        throw new Exception(Mage::helper('etep')->__('E-Transactions not available. Please try again later.'));
    }
    ...
    public function getParams($logParams = false, $checkSign = true) {
        ...
        if (empty($data)) {
            ...
            Mage::throwException($helper->__('An unexpected error in E-Transactions call has occured: no parameters.'));
        }
        ...
        if ($checkSign) {
            ...
            if (!preg_match('#^(.*)&K=(.*)$#', $data, $matches)) {
                $helper = Mage::helper('etep');
                Mage::throwException($helper->__('An unexpected error in E-Transactions call has occured: missing signature.'));
            }
        ...
        if (empty($params)) {
            $helper = Mage::helper('etep');
            Mage::throwException($helper->__('An unexpected error in E-Transactions call has occured.'));
        }
    ...
    public function untokenizeOrder($token) {
        ...
        if (count($parts) < 2) {
            ...
            Mage::throwException(Mage::helper('etep')->__($message, $token));
        }
